  ...
  if (resolved === null) {
    ...
    if (asIndex) {
      return Promise.resolve(
        findColorIndexByColorValue(screenData, empty)
      );
    }
...
function filterImg(screenData, options) {
  ...
  let x1 = getInt(options.x1, 0);
  let y1 = getInt(options.y1, 0);
  let x2 = getInt(options.x2, viewSnap.width - 1);
  let y2 = getInt(options.y2, viewSnap.height - 1);
  ...
  for (const binding of Object.values(bindings)) {
    ...
    for (const source of binding.sources) {
      if (!textures.has(source)) {
        textures.set(source, getWebGL2Texture(screenData, source));
      }
    }
  }
  return textures;
...
function setDisplayShader(screenData, options) {
  ...
  screenData.displayShaderTextureResolver = (source) => {
    return getWebGL2Texture(screenData, source);
  };
