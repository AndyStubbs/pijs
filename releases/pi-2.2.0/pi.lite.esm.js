...
__export(commands_exports, {
  ...
  processScreenCommands: () => processScreenCommands,
  set: () => set,
  wait: () => wait
});
...
__export(screen_manager_exports, {
  ...
  addScreenPreCleanupFunction: () => addScreenPreCleanupFunction,
  assertScreenAvailable: () => assertScreenAvailable,
  getActiveScreen: () => getActiveScreen,
  getAllScreensData: () => getAllScreensData,
  getScreenData: () => getScreenData,
  init: () => init14,
  installScreenExtensions: () => installScreenExtensions,
  presentCurrentScreen: () => presentCurrentScreen,
  refreshScreenSize: () => refreshScreenSize,
...
__export(renderer_exports, {
  ...
  cleanup: () => cleanup3,
  cls: () => cls,
  countQueuedShaderPasses: () => countQueuedShaderPasses,
  createContext: () => createContext,
  createPointWriter: () => createPointWriter,
  deleteCustomShaderProgram: () => deleteCustomShaderProgram,
  ...
  getCustomShaderDiagnostics: () => getCustomShaderDiagnostics,
  getSamplerTexture: () => getSamplerTexture,
  getWebGL2Texture: () => getWebGL2Texture,
  init: () => init9,
  normalizeCustomUniforms: () => normalizeCustomUniforms,
  ...
  validateCustomShaderProgram: () => validateCustomShaderProgram
});
...
__export(postfx_exports, {
  ...
  init: () => init2,
  restoreDisplayShaderBindings: () => restoreDisplayShaderBindings
});
...
function getShaderHandle(shaderHandle, cmdName) {
  ...
  if (shaderHandle == null) {
    const error2 = new TypeError(`${cmdName}: Parameter shaderHandle is required.`);
  ...
  if (typeof shaderHandle === "number") {
    ...
    if (!handle) {
      const error2 = new TypeError(`${cmdName}: Unknown shader handle id ${shaderHandle}.`);
      error2.code = "INVALID_SHADER_HANDLE";
      throw error2;
    }
    return handle;
...
function resolveSamplerSource(screenData, input, cmdName) {
  let source;
  try {
    if (m_screenCanvasMap.has(input)) {
      source = input;
    } else {
      source = getImageFromRawInput(input, cmdName);
    }
  } catch (error) {
    if (error.code === "INVALID_NAME") {
      const uniformError2 = new TypeError(`${cmdName}: Invalid sampler2D image input.`);
      uniformError2.code = "INVALID_UNIFORM_VALUE";
      throw uniformError2;
    }
    throw error;
  }
...
__export(view_exports, {
  ...
  recomputeViewCache: () => recomputeViewCache,
  snapshotView: () => snapshotView,
  syncFullScreenCache: () => syncFullScreenCache,
  toScreen: () => toScreen
});
...
function initializePlugin(pluginInfo) {
  ...
  const existingScreens = getAllScreensData();
  const extensions = {
    "commands": [],
    "dataItems": [],
    "dataItemGetters": [],
  ...
  const pluginApi = {
    "addCommand": (...args) => {
      extensions.commands.push(addCommand(...args));
    },
  ...
  try {
    ...
    installScreenExtensions(existingScreens, extensions);
    processCommands(m_api3, extensions.commands);
...
var m_activeFilters = /* @__PURE__ */ new WeakMap();
function init17(api) {
  registerCommands9();
  addScreenPreCleanupFunction(cancelFilter);
...
//# sourceMappingURL=pi.lite.esm.js.map
