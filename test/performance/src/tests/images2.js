...
function generateOperationList() {
	...
	for( let i = 0; i < 1000; i++ ) {
		const operation = generateRandomOperation2();
...
function generateRandomOperation() {
	...
	if( m_useSprite ) {
		...
		name = g_sprites[ Math.floor( m_seededRandom() * g_sprites.length ) ];
	} else {
		drawFn = screen.blitImage;
		name = g_images[ Math.floor( m_seededRandom() * g_images.length ) ];
	}
	...
	let color = $.getPalColor( colorIndex );
	...
	const params = [];
	params.push( $.getImage( name ) );
	params.push( x );
	params.push( y );
	const variants = [
		Math.max( 0, Math.min( x + Math.floor( Math.random() * 6 ) - 3, width ) ),
		Math.max( 0, Math.min( y + Math.floor( Math.random() * 6 ) - 3, height ) )
	];

	if( m_useSprite ){
		const spriteInfo = $.getSpritesheetData( name );
	...
	return {
		...
		"getParams": () => {
			...
			if( m_useColor ) {
				...
				newParams.push( undefined );
			}
			if( includeAnchor ) {
				newParams.push( newAnchorX );
				newParams.push( newAnchorY );
			} else {
				newParams.push( undefined );
				newParams.push( undefined );
			}
			if( includeScale ) {
				newParams.push( newScaleX );
				newParams.push( newScaleY );
			} else {
				newParams.push( undefined );
				newParams.push( undefined );
			}
			if( includeAngle ) {
				newParams.push( newAngle );
			} else {
				newParams.push( undefined );
			}
...
function generateRandomOperation2() {
	const screen = $.getScreen( 0 );
	const width = screen.width();
	const height = screen.height();
	let drawFn;
