...
const CONFIG = {
	...
	"dependencies": [ "pointer", "keyboard", "print-table" ],
...
export default function onscreenKeyboardPlugin( pluginApi ) {
	...
	pluginApi.addCommand( "showKeyboard", showKeyboard, true, [ "mode" ] );

	// Add hideKeyboard command
	pluginApi.addCommand( "hideKeyboard", hideKeyboard, true, [] );
	...
	function hideKeyboard( screenData ) {
		...
		if( m_keyboardState.background && m_keyboardState.startPosPx ) {
			const posPx = m_keyboardState.startPosPx;

			// Restore background
			screenData.api.blitImage( m_keyboardState.background, posPx.x, posPx.y );

			// Delete image
			m_api.removeImage( m_keyboardState.backgroundName );
		}
	...
	function renderKeyboard() {
		...
		if( m_keyboardState.background === null ) {	
			...
			screenData.api.createImageFromScreen( {
				"name": m_keyboardState.backgroundName ,
				"x1": x,
				"y1": y,
				"x2": x + width,
				"y2": y + height
			} );
			m_keyboardState.background = m_api.getImage( m_keyboardState.backgroundName );
		} else {

			// Restore previous background before rendering new keyboard
			screenData.api.cls( x, y, width, height );
			screenData.api.blitImage( m_keyboardState.background, x, y );
