...
- `API.md` and the hand-written llms references (`llms.txt`, `llms-full.txt`, `examples.txt`)
  are updated only in the 2.3 release phase (general plan Section 11), to describe final
  behavior. Plan documents record intermediate decisions.
- `docs/llms/pi.d.ts` is generated by every normal `npm run build`, so it cannot wait for the
  release phase. Commit the regenerated file with each API change, as the existing metadata and
  type tests require.
...
the 2.3 release phase, and the frozen `releases/pi-2.2.0` snapshot is untouched.
...
- **D5** is resolved as `"ambient"`, not the recommended `"playback"`: sound follows the iOS
  mute switch and mixes with other apps' audio (plan 12). The release phase documents it.
...
Goal: decide from measured size which `sound-advanced` modules move into core, and close the
sound workstream. The release tasks (API documentation, llms references, the upgrade guide,
version checks, and the release snapshot) are tasks R.1–R.7 of the general plan, because they
cover every 2.3 workstream.

| # | Task | Plan § |
| --- | --- | --- |
| 6.1 | Size review from the differential report (promotion and full-merge costs); core-promotion decision per module; resolve D4 | 9.1, 12 |
| 6.2 | Apply any promotions or full-build inclusion decided in 6.1 | 9.1 |

Exit criteria:

- D4 and every other sound open decision are closed.
- The size decisions and any remaining variance are recorded in
  `docs/evidence/sound-2.3/README.md`.
